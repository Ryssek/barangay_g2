...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
  if (isset($_GET['search']) && strlen($_GET['search']) > 0) {
    ...
    $sql = "SELECT * FROM clearances WHERE resident_name LIKE '%$search_query%' OR clearance_type LIKE '%$search_query%'";
  ...
  <div class="min-vh-100 p-3" style="width: 83%">
    <div class="container-fluid">
      <div id="directory_link" class="p-3 rounded-4 bg-dark-subtle ">
        ...
        <a href="manage_clearances.php" class="text-danger text-decoration-none">Manage Clearances</a>
        <a href="search_clearances.php" class="text-decoration-none text-black-50">/ Search Clearances</a>
      </div>
      <div class="d-flex align-items-center justify-content-between">
        <h1 class="text-danger">Search Clearances</h1>
      ...
      <form class="d-flex gap-2" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="GET">
        <input class="form-control" name="search" id="search" type="text" placeholder="Search by resident name or clearance type" style="border: 1px solid gray" />
          ...
          <table class="table table-dark table-striped table-hover table-bordered text-center align-middle">
            <thead class="table-secondary">
              <tr>
                ...
                <th class="text-nowrap">Clearance Type</th>
            ...
            <tbody class="table-group-divider">
              ...
              if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                  ...
                  echo "<td> <div class='d-flex align-items-center justify-content-center gap-2 h-100'> <a href='view_clearance_details.php?id={$row['id']}' class='btn btn-primary'>View</a>  <a href='update_clearance.php?id={$row['id']}' class='btn btn-danger'>Edit</a>  <a href='update_res.php?id={$row['id']}' class='btn btn-light'>Print</a>   </div> </td>";
